    ...
    if self.style == "dashed" then
        ...
        if self.empty_segments then
            ...
            paintRect(x + self.empty_segments[1].e, y,
                self.dimen.w - self.empty_segments[1].e,
                self.dimen.h,
                self.background
            )
        else
...
local function buildMonthlyChart(popup_self, monthly_data, layout, fonts)
    ...
    local function createBarRow(data_slice)
        ...
        for i, m in ipairs(data_slice) do
            ...
            local bar_color
            if is_current then
                bar_color = Blitbuffer.colorFromString(InsightColors.currMonthColor)        --curernt month gets specified color
            else
                bar_color = BAR_COLORS_RGB[math.random(#BAR_COLORS_RGB)]                    --other months get randomnized from selection
            end

            --only show label if value > 0
            local value_label = TextWidget:new{
                text = value > 0 and formatNumber(value) or "",
                face = font_small,
            }
...
local function buildCurrentStreakWidget(streaks_dimen, value, weeksOrDays, fonts, streaks_colors)
	...
	return FrameContainer:new{
			...
			bordersize = 0,
			margin = 0,
			-- color = weeksOrDays == 0 and streaks_colors.midGray or streaks_colors.lightGray,
            -- background = weeksOrDays == 0 and streaks_colors.midGray or streaks_colors.lightGray,
            background = weeksOrDays == 0 and Blitbuffer.colorFromString(InsightColors.weeksColor) or Blitbuffer.colorFromString(InsightColors.daysColor),
			radius = Screen:scaleBySize(7),
			CenterContainer:new{
...
local function buildBestStreakWidget(streaks, streaks_dimen, fonts, streaks_colors)
	...
	local function buildBestModule(value, weekOrDay, isLongest, ts_start, ts_end)
		...
		local heading_text_widget = TextBoxWidget:new{
								...
								text = heading_text,
								face = fonts.streakLabel,
								fgcolor = Blitbuffer.colorFromString(InsightColors.recordColor),
		}
		...
		local value_widget = TextBoxWidget:new{
									...
									fgcolor = Blitbuffer.colorFromString(InsightColors.valueTextColor),
		}		
